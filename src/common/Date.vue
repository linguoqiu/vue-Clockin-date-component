<template>
  <div class="date">
    <div class="wrap">
      <div class="cal-wrap">
        <button id="prevMonth" class="month-btn" @click="prevMonth"><</button>
        <h2 class="tpl-date">{{showYear}}年{{showMonth}}月</h2>
        <button id="nextMonth" class="month-btn" @click="nextMonth">></button>
      </div>
      <ul class="week-f">
        <li class="wk">日</li>
        <li>一</li>
      ...
      <div id="calendar" v-html="calendardiv">{{calendardiv}}</div>
    </div>
  </div>
</template>
  ...
  data () {
    return {
      ...
      util: {
            ...
            dateToNum: function( date ) {
              ...
              if ( typeof date == 'string' ) {
                ...
                rst = this.formatDate( date ).split( '-' ).join( '' );
              }
            ...
            getNextDate: function( date, num ) {
              return this.util.formatDate( new Date( +new Date( date ) + num * 86400000 ) );
            },
      },
      tpl : [
          '<div class="cal-wrap">',
      ...
      holidaysMap : [
          {
            name: 'normal',//完成
            date: [ '2017-06-02', '2017-06-10', '2017-06-05', '2017-06-15' ]
          },
          {
            name: 'late',//迟到
            date: [ '2017-06-03', '2017-06-13', '2017-06-18', '2017-06-09']
          },
          {
            name: 'leave',//早退
            date: [ '2017-06-20', '2017-06-11', '2017-06-08', '2017-06-16']
          },
          {
            name: '圣诞节',
    ...
    render: function( date ) {
      ...
      for ( var i = 0; i < config.count; i++ ) {
        var curMonth = month + i,
                curDate = this.util.getNextMonthDate( year, curMonth ),
                dateStr = curDate.year + '年' + this.util.formatNum( curDate.month ) + '月',
        ...
        var curTpl = this.tpl.replace( '{%date%}', dateStr )
                .replace( '{%week%}', week )
                .replace( '{%day%}', dayList );
      ...
      //vue中使用v-html
      this.calendardiv = tmpTplArr.join( '' );
...
//          }
      ...
      //插入空白字符,如果星期天排最后的话，条件为：i < firstDay-1，并将上面的代码打开
      ...
      for ( var i = 0; i < days; i++ ) {
        var day = i + 1,
                date = this.util.joinDate( year, month, day ),
                wkDay = new Date( date ).getDay(),
                dateNum = this.util.dateToNum( date ),
                jrClassName = 'dl jr';
        ...
        //不在指定范围内的日期置灰
        if ( ( me.minDate && dateNum < this.util.dateToNum( me.minDate ) ) ||
                ( me.maxDate && dateNum > this.util.dateToNum( me.maxDate ) )
        ) {
          jrClassName = 'dl iv';
        ...
        if ( config.isShowHoliday ) {
          for ( var k = 0, hlen = this.holidaysMap.length; k < hlen; k++ ) {
            var name = this.holidaysMap[ k ][ 'name' ],
                    dateArr = this.holidaysMap[ k ][ 'date' ];
      ...
      for ( var i = 0, len = tmpDayDataArr.length; i < len; i++ ) {
        ...
        if(item.name == 'normal'){
          tmpDayTplArr.push(
                  ...
                  '<div class="normal-bg">' + item.day + '</div>' +
                  '</li>'
          );
        }else if(item.name == 'late'){
          tmpDayTplArr.push(
                  '<li class="" data-date="' + item.date + '">' +
                  '<div class="late-bg">' + item.day + '</div>' +
                  '</li>'
          );
        }else if(item.name == 'leave'){
          tmpDayTplArr.push(
                  '<li class="" data-date="' + item.date + '">' +
                  '<div class="leave-earlier-bg">' + item.day + '</div>' +
                  '</li>'
          );
        }else{
          tmpDayTplArr.push(
    ...
    setSelectDate: function( date ) {
      var me = this,
              config = this.config,
              date = ( typeof date == 'string' ) ? date : this.util.formatDate( date ),
              dateNum = this.util.dateToNum( date ),
...
ul.day .normal-bg{
  ...
  border-radius: 50%;
  margin: 0 auto;
  /*background-image: -moz-radial-gradient(45px 45px 45deg, circle cover, #85CF56 0%, orange 100%, red 95%);*/
  /*background-image: -webkit-radial-gradient(45px 45px, circle cover, #85CF56, orange);*/
  /*background-image: radial-gradient(45px 45px 45deg, circle cover, #85CF56 0%, orange 100%, red 95%);*/
  animation-name: spin;
